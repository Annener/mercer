...
# ---------------------------------------------------------------------------
# Vault file helpers (mark_file_pending / get_vault_file_entry /
# remove_file_from_vault_cache / get_all_vault_file_entries)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_mark_file_pending_sets_status(mgr: RedisStateManager) -> None:
...
@pytest.mark.asyncio
async def test_mark_file_pending_creates_entry_if_missing(mgr: RedisStateManager) -> None:
...
@pytest.mark.asyncio
async def test_remove_file_from_vault_cache(mgr: RedisStateManager) -> None:
    await mgr.mark_file_indexed("v1", "a.md", "abc", 5)
...
@pytest.mark.asyncio
async def test_get_vault_file_entry_returns_none_for_missing(mgr: RedisStateManager) -> None:
    entry = await mgr.get_vault_file_entry("v1", "nonexistent.md")
    assert entry is None


@pytest.mark.asyncio
async def test_get_all_vault_file_entries_skips_empty_sentinel(mgr: RedisStateManager) -> None:
...
@pytest.mark.asyncio
async def test_get_all_vault_file_entries_returns_all(mgr: RedisStateManager) -> None:
