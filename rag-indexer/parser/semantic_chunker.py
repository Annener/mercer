...
Примечание по MAX_CHUNK_CHARS:
  Кириллический текст токенизируется с коэффициентом ~1.5–2 токена/символ.
  При batch size Ollama = 2048 токенов безопасный лимит: ~1500 символов
  (≈ 750–1000 токенов с запасом). Не поднимать выше 1800 без увеличения
  num_batch в конфиге Ollama (параметр --num-batch).
"""
...
class SemanticChunker:
    ...
    # Лимит символов на чанк.
    # ВАЖНО: кириллица токенизируется ~1.5–2 токена/символ.
    # При Ollama num_batch=2048: безопасный лимит ≈ 1500 символов (~750–1000 токенов).
    # Если увеличить num_batch в Ollama (--num-batch), можно пропорционально поднять.
    MAX_CHUNK_CHARS: int = 1500

    # При достижении SOFT_THRESHOLD начинаем искать конец предложения
    SOFT_THRESHOLD: int = 1000
