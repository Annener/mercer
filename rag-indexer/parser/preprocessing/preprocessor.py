...
# Паттерн Markdown-заголовка (строка начинается с # ... ####### + пробел)
...
def preprocess(text: str, source_hint: str = "") -> str:
    ...
    # 4a. Склеивание дефисных переносов когда \n уже заменён пробелом:
    # "выва- ливается" → "вываливается", "едино- рог" → "единорог"
    # Срабатывает ТОЛЬКО когда пробел стоит после дефиса в конце слова.
    # Не затрагивает составные слова ("эльф-обыватель"), даты ("2023-01-01"),
    # ISBN и числа с дефисом ("978-5-04") — у них нет пробела после дефиса.
    text = re.sub(r"(\w+)-\s+(\w)", r"\1\2", text)

    # 4b. Гарантируем \n\n вокруг Markdown-заголовков ДО схлопывания одинарных \n.
    # Проблема: в MD и PDF-источниках заголовок вида "# N игра" может быть отделён
    # от предыдущего абзаца лишь одинарным \n. Шаг 5 ниже превратил бы его
    # в пробел, и заголовок слился бы в одну строку с предыдущим текстом.
    # SemanticChunker распознаёт границы только по \n\n → чанк получался гигантским.
    # Решение: заменяем \n#{1,6}\s на \n\n#{1,6}\s (и зеркально после заголовка),
    # чтобы каждый заголовок гарантированно был обёрнут пустыми строками.
    text = _HEADING_LINE_RE.sub(r"\n\n\1", text)
