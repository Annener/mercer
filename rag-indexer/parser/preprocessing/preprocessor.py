...
def preprocess(text: str, source_hint: str = "") -> str:
    ...
    # 4b. Гарантируем \n\n ВОКРУГ каждой строки Markdown-заголовка ДО схлопывания
    # одинарных \n в шаге 5.
    #
    # Проблема: заголовок вида "# N игра" может быть отделён от предыдущего абзаца
    # лишь одинарным \n, а строка ПОСЛЕ заголовка тоже разделена одинарным \n.
    # Шаг 5 ниже превращает оба \n в пробелы → заголовок сливается с соседними
    # абзацами в одну строку, SemanticChunker не видит границы → чанк гигантский.
    #
