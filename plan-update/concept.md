# Campaign Update Mode — Концепт

## Цель

Campaign Update Mode добавляет в Mercer управляемый процесс актуализации markdown-контекста кампании.
...
Это позволяет использовать retrieval и не отправлять в модель весь campaign vault.
...
Indexer хранит краткоживущий результат применённого `apply_id` в Redis или другом доступном shared state. Повторный запрос с тем же `apply_id` возвращает тот же результат и не создаёт второй commit.
...
Identity передаётся process-local в subprocess environment:
...
- Если среди target paths есть изменения, создаётся commit:

```text
snapshot: manual edits before campaign update
```
...
Apply не является распределённой транзакцией между разными git-repositories.

Изменения группируются и применяются по vault. Возможен частичный успех.
...
| Максимальное число enabled vault domain | не лимитируется продуктово |
...
## Проверяемые критерии готовности MVP

1. Без campaign tags update mode не стартует.
2. Поиск ограничен indexed `.md` документами campaign tags и enabled vault domain.
3. LLM получает full indexed text только выбранных документов.
4. LLM не возвращает свободный абсолютный путь и не генерирует полный файл для overwrite.
5. Indexer единолично читает оригинальный файл, строит diff, пишет файл и работает с git.
6. UI показывает diff оригинального файла.
7. Apply не затирает файл, изменённый после review.
8. Create работает рядом с parent file или в `_campaign_notes/`.
9. Git stage включает только explicit accepted `.md` paths.
10. После commit запускается targeted reindex и UI получает task IDs.
11. Multi-vault partial success прозрачно возвращается и отображается.
12. Все новые API, contracts, DB fields, migrations и context documents покрыты тестами и обновлены.
