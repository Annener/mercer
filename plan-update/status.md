...
```text
Состояние: Все фазы 0–5 завершены. Campaign Update Mode полностью реализован.
Текущая фаза: —
...
- [x] UI (Phase 5): Update Mode review/apply интерфейс реализован и интегрирован.
...
- **2.10** — `IndexerUnavailableError` маппится в HTTP **502** (Bad Gateway), тогда как в docstring модуля аналогичная ошибка `UpdateModeIndexerUnavailableError` помечена как 503. Семантически корректно (upstream недоступен = bad gateway), но расходится с docstring. Рекомендуется выровнять при рефакторинге — не блокирует.
...
| 5 | `phase-5-sse-frontend.md` | UI, E2E, deployment, observability | **Done** |
...
|---|---|---|
| `POST /api/chats/{chat_id}/update-mode/start` | Note → retrieval → LLM intents → resolved diffs | **Ready** |
...
| `POST /api/chats/{chat_id}/update-mode/apply` | Применить accepted changes + targeted reindex + audit log | **Ready** |
...
| Endpoint | Назначение | Состояние |
|---|---|---|
| `POST /internal/update-mode/resolve` | Intent → original-file diff | **Ready** |
| `POST /internal/update-mode/apply` | Checksum → snapshot → write → commit → targeted reindex | **Ready** |
...
- [x] Все acceptance criteria фаз 0–5 закрыты.
- [x] Migration применена и downgrade проверен.
- [x] Backend, indexer и frontend tests проходят.
- [x] Docker E2E update существующего файла проходит.
- [x] Docker E2E create рядом с parent проходит.
- [x] Docker E2E fallback `_campaign_notes` проходит.
- [x] Multi-vault partial conflict корректно отображается.
- [x] Stale checksum не перезаписывает ручную правку.
- [x] Apply retry с same `apply_id` создаёт ровно один commit.
- [x] Unrelated dirty files не попадают в snapshot/apply commit.
- [x] Targeted reindex task успешно обновляет changed documents.
- [x] AuditLog не содержит file content/diff/note.
- [x] Deployment documentation описывает git, mounts, limits и security assumptions.
