...
- [x] **A4** — Удалены файлы: `state_manager.py` и `embedding/cache.py` (после проверки импортов)
  - Дата: 2026-06-27
  - Заметки: grep подтвердил — оба файла не импортируются. Файлы отсутствовали в репо ещё до сессии 2
...
- [x] **C1** — Добавлен диспетчер `_agent-setup-dispatch`, текущий `agent-setup` переименован в `_agent-setup-launchd`
  - Дата: 2026-06-28
  - Заметки: `setup` вызывает `_agent-setup-dispatch`; host/docker/win разветвление; коммит 738fca1

- [x] **C2** — `HOST_AGENT_TOKEN` раскомментирован в plist-шаблоне и подставляется через `_render-plist`
  - Дата: 2026-06-28
  - Заметки: placeholder `{{HOST_AGENT_TOKEN}}` в plist; sed-подстановка из `.env`; коммиты 07f252a + 738fca1
