...
| 7 | rag-indexer: indexer_service — async cancel, убрать broadcaster | [step-07](step-07-indexer-service.md) | ✅ завершён | f0e1330 | Удалены _broadcaster, _cancel_flags, get_broadcaster. cancel_task → async, записывает Redis-флаг. __init__ принимает state_manager: RedisStateManager. _on_task_done чистит _tasks. Добавлен shutdown(). Тесты: tests/rag_indexer/test_indexer_service.py |
...
| 2026-06-21 | 7 | ✅ indexer_service: удалены _broadcaster, _cancel_flags, get_broadcaster. cancel_task → async + Redis-флаг. __init__ принимает state_manager. _on_task_done чистит _tasks. Добавлен shutdown(). Тесты: tests/rag_indexer/test_indexer_service.py |
