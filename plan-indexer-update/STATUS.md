...
| 11 | Интеграционный тест | [step-11](step-11-integration-test.md) | ✅ завершён | ручной тест | vault=dnd, 51 файл. status: running→done. TTL task≈86400, TTL vault=-1. grep: только тест-код |
...
| 2026-06-21 | 11 | ✅ Интеграционный тест пройдён на живом стеке. vault=dnd, 51 файл. Polling: status running→done, files_done 2→3, chunks 0→398 (Поворот колеса фортуны.pdf). vault-кэш: 51 файл, by_status={indexed:51}. TTL task≈86400с, TTL vault=-1. grep: продукционный код WS не содержит |
