...
# Путь к директории pdf-sidecar: можно переопределить через переменную окружения.
# По умолчанию — ../pdf-sidecar относительно этого файла.
_DEFAULT_SIDECAR_DIR = Path(__file__).parent.parent / "pdf-sidecar"
SIDECAR_DIR: Path = Path(os.getenv("SIDECAR_DIR", str(_DEFAULT_SIDECAR_DIR))).resolve()
...
async def _stream_script(script: str) -> AsyncIterator[str]:
    ...
    try:
        async for raw_line in proc.stdout:
            ...
            yield f"data: {line}\n\n"
            await asyncio.sleep(0)  # даём event loop выдохнуть
    finally:
...
async def sidecar_restart(x_agent_token: str | None = Header(default=None)) -> JSONResponse:
    check_token(x_agent_token)
    # stop (игнорируем ошибку если уже остановлен)
...
async def sidecar_install_stream(
    ...
    async def event_stream() -> AsyncIterator[str]:
        # Начальное событие чтобы соединение сразу открылось
        yield "data: [START] Running install.sh...\n\n"
        async for chunk in _stream_script("install.sh"):
            # Проверяем disconnect каждую итерацию
