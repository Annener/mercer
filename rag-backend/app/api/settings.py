...
async def list_worlds(vault_id: str | None = None, db: AsyncSession = Depends(get_db)) -> list[dict[str, Any]]:
    ...
    return [_world_dict(world) for world in result.scalars().all()]
...
async def create_campaign(world_id: str, req: CampaignCreateRequest, db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    ...
    if not req.path_prefix.startswith(world.path_prefix) or not req.path_prefix.endswith("/"):
        raise HTTPException(status_code=422, detail="campaign path_prefix must be inside world path_prefix and end with /")
...
async def list_pipelines(domain_id: str | None = None, db: AsyncSession = Depends(get_db)) -> list[dict[str, Any]]:
    ...
    result = await db.execute(stmt)
    return [_pipeline_dict(pipeline) for pipeline in result.scalars().all()]
...
def _vault_dict(vault: Vault) -> dict[str, Any]:
    return {
        ...
        "overlap": vault.overlap,
        "entity_aware_mode": vault.entity_aware_mode,
        "binding_status": vault.binding_status,
...
def _world_dict(world: World) -> dict[str, Any]:
    return {
        "id": str(world.id),
        "world_id": world.world_id,
        "vault_id": world.vault_id,
        "name": world.name,
        "description": world.description,
        "path_prefix": world.path_prefix,
...
def _campaign_dict(campaign: Campaign) -> dict[str, Any]:
    return {
        ...
        "path_prefix": campaign.path_prefix,
...
def _pipeline_dict(pipeline: Pipeline) -> dict[str, Any]:
    return {
        "id": str(pipeline.id),
        "pipeline_id": pipeline.pipeline_id,
        ...
        "final_composition": pipeline.final_composition,
...
def _validate_pipeline_json(steps: list[dict[str, Any]], final_composition: dict[str, Any]) -> None:
    ...
    for step in steps:
        for key in ("order", "type", "name", "role", "system_prompt"):
            if key not in step:
                raise HTTPException(status_code=422, detail=f"Pipeline step missing {key}")
