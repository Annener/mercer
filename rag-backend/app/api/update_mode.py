...
chat_id is the primary key for all operations — it is generated by /start
and returned to the client. All subsequent requests use it as a path param.
campaign_id is passed in the /start request body and stored in the session.
...
async def start_update_mode(
    ...
    chat_id is provided by the client as a path param — it becomes the Redis
    session key. campaign_id is supplied in the request body.
    ...
    async with SessionLocal() as db:
        domain_id, vault_ids, default_vault_id, candidate_doc_ids = (
            await _get_campaign_context(db, body.campaign_id)
        )
