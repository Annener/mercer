const DocumentsTabMixin = {
    ...
    // Каждый узел: { _isDir: true, children: {name: node} } | { _isDir: false, doc }
    _buildDocsTree(docs) {
    ...
    // Возвращает Set путей директорий-предков совпадающих файлов (для авто-раскрытия при поиске).
    ...
    // Рекурсивный рендер узлов дерева в tbody-контейнер.
    _renderDocsTree(node, container, depth, openDirs) {
        ...
        for (const [name, child] of entries) {
            if (child._isDir) {
                ...
                dirRow.innerHTML = `
                    <td colspan="5" class="docs-dir-cell" style="padding-left:${8 + depth * 18}px;">
                ...
                row.innerHTML = `
                    ...
                    <td><button class="btn btn-sm" style="color:var(--color-error);" data-action="delete-doc"
                        data-id="${this.escapeHtml(String(doc.id))}"
                        data-vault="${this.escapeHtml(doc.vault_id || '')}"
                        data-path="${this.escapeHtml(doc.source_path || String(doc.id))}"
                        title="Удалить">🗑</button></td>`;

                row.addEventListener('click', e => {
                    if (e.target.closest('[data-action="delete-doc"]')) return;
                    this._openDocModal(doc);
                });
                row.querySelector('[data-action="delete-doc"]').addEventListener('click', e => {
                    e.stopPropagation();
                    this.handleDocumentsAction('delete-doc', e.currentTarget);
                });
    ...
    _renderDocsRows(docs) {
        ...
        // Удаляем все tbody (оставляем только thead)
        tableEl.querySelectorAll('tbody').forEach(b => b.remove());

        if (!docs || !docs.length) {
            const empty = document.createElement('tbody');
            empty.innerHTML = '<tr><td colspan="5" class="empty-state">Документов нет</td></tr>';
        ...
        // При поиске — авто-раскрываем предков совпадений; иначе — сохранённое состояние
