const CampaignsTabMixin = {
    async renderCampaignsTab() {
        ...
        const toolbar = `<div class="settings-toolbar">
            <button class="btn btn-primary" data-action="new-campaign">+ Новый контекст</button>
        </div>`;

        const cardsHtml = !campaigns.length
            ? toolbar + '<div class="empty-state">Контекстов нет, создайте новый</div>'
    ...
    async showCampaignModal(campaignId = null) {
        ...
        overlay.innerHTML = `<div class="modal-content" style="max-width:560px;width:100%;max-height:90vh;overflow-y:auto;">
            <div class="modal-header">
                <h3>${isEdit ? 'Редактировать' : 'Новый контекст'}</h3>
            ...
            <div class="form-group" style="padding:0 1.5rem;">
                ...
                <textarea id="camp-prompt" class="input-field" rows="5" placeholder="Инструкция для AI в этом контексте">${this.escapeHtml(campaign.system_prompt || '')}</textarea>
            </div>
            ${isEdit ? `
            <div class="form-group" style="padding:0 1.5rem;">
