class SettingsManager {
    ...
    async renderPipelinesTab() {
        ...
        return toolbar + `<div class="settings-grid">${pipelines.map(pipeline => `
            <article class="settings-card">
                ...
                <div class="settings-actions">
                    <button class="btn btn-sm btn-secondary" data-action="edit-pipeline" data-id="${this.escapeHtml(pipeline.id)}">✏️ Редактировать</button>
                    ...
                    <button class="btn btn-sm btn-danger" data-action="deactivate-pipeline" data-id="${this.escapeHtml(pipeline.id)}">Деактивировать</button>
                </div>
    ...
    async renderWorldsTab() {
        ...
        if (worldsArr.length === 0) return toolbar + `<div class="empty-state">Миров нет</div><p class="settings-note">Удаление миров выполняется в файловой системе.</p>`;
    ...
    // ИСПРАВЛЕНИЕ: фильтруем домены по enabled:true при создании vault
        ...
        modal.innerHTML = `
            <div class="modal-content">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="world-path-input" placeholder="worlds/myworld/">
                </div>
                <div class="modal-actions">
